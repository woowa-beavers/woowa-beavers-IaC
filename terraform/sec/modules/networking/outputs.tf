...
# 역할: networking 모듈 출력값 - VPC/서브넷/NAT 정보 노출
# 흐름: main.tf 리소스·데이터 소스 → 이 파일 출력값 → 상위 루트 모듈 참조
...
output "misp_public_subnet_id" {
  ...
  value       = data.aws_subnet.misp_public.id
}

output "misp_private_subnet_id" {
  description = "MISP 프라이빗 서브넷 ID"
  value       = data.aws_subnet.misp_private.id
}
