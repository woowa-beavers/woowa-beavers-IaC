...
| ops | GitHub Actions OIDC, CI/CD, Terraform 실행 주체 | 미작성 |
| dev | 쇼핑몰 인프라, S3 | 완료 |
| billing | 비용 알림 | - |
...
├── terraform/
│   ├── modules/                    # 공통 모듈 (dev·staging·prod 공유)
...
│   ├── sec/                        # sec 계정 - 보안 서버
│   │   ├── modules/
│   │   │   ├── compute/            # TheHive NAT 인스턴스·서버 EC2
│   │   │   ├── networking/         # MISP VPC/서브넷 조회, NAT Gateway
│   │   │   ├── security/           # GuardDuty, SecurityHub, Config, KMS (미작성)
│   │   │   └── iam/                # TheHive SSM IAM Role/Profile
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── terraform.tfvars.example
│   ├── ops/                        # ops 계정 - CI/CD, OIDC (미작성)
│   │   └── main.tf
│   ├── root/                       # root 계정 - Organizations OU·계정 관리, CloudTrail
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   └── outputs.tf
│   └── cloudflare/                 # Cloudflare - DNS 레코드, Tunnel CNAME
│       ├── main.tf
│       ├── variables.tf
│       ├── outputs.tf
│       └── terraform.tfvars.example
...
```bash
cd terraform/environments/dev   # 또는 terraform/environments/staging, terraform/environments/prod
