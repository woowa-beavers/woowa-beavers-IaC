...
root · sec · ops · dev 계정의 AWS 멀티 계정 인프라를 코드로 관리하며,  
...
| 클라우드 | ![AWS](https://img.shields.io/badge/AWS_멀티계정-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white) |
...
| 계정 | 역할 | Terraform |
|:---:|:---|:---:|
| root | Organizations OU·계정 관리, CloudTrail | 완료 |
| sec | TheHive, MISP, GuardDuty, SecurityHub, Config, S3 | 완료 |
| ops | GitHub Actions OIDC, Terraform 실행 IAM Role | 완료 |
| dev | 쇼핑몰 인프라 (VPC·EC2·ALB·RDS·S3·CloudFront) | 완료 |
| billing | 비용 알림 | - |
| cloudflare | DNS 레코드, Tunnel CNAME | 완료 |
...
├── .github/
│   └── workflows/
│       └── terraform.yml           # PR: fmt·validate / 수동: plan·apply (멀티 계정)
│
├── terraform/
│   ├── modules/                    # dev 공용 모듈
│   │   ├── networking/             # VPC·서브넷·IGW·라우트 테이블·Bastion·NAT Instance·ALB SG·RDS SG
...
│   ├── sec/                        # sec 계정 루트 모듈
│   │   ├── modules/
│   │   │   ├── networking/         # MISP VPC·서브넷·NAT Gateway
│   │   │   ├── compute/            # TheHive·MISP EC2 및 보안그룹
│   │   │   ├── security/           # GuardDuty·SecurityHub·Config·KMS
│   │   │   └── iam/                # TheHive SSM IAM Role·인스턴스 프로파일
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── terraform.tfvars.example
│   │
│   ├── ops/                        # ops 계정 루트 모듈
│   │   ├── main.tf                 # GitHub Actions OIDC Provider·Terraform 실행 IAM Role
│   │   ├── variables.tf
│   │   └── outputs.tf
│   │
│   ├── root/                       # root 계정 루트 모듈
│   │   ├── main.tf                 # Organizations OU·계정 관리·CloudTrail
│   │   ├── variables.tf
│   │   └── outputs.tf
│   │
│   └── cloudflare/                 # Cloudflare DNS·Tunnel CNAME
│       ├── main.tf
│       ├── variables.tf
│       ├── outputs.tf
│       └── terraform.tfvars.example
...
│   ├── ansible.cfg
│   ├── inventory/
│   │   ├── hosts.yml               # sec·홈서버 호스트 목록
│   │   └── group_vars/
│   │       └── all.yml             # 공통 변수 (IP·키·토큰 등)
│   ├── playbooks/
│   │   └── site.yml                # MISP·TheHive·Wazuh 통합 플레이북
│   └── roles/
│       ├── misp/                   # MISP + Cloudflare Tunnel Docker Compose 배포
│       ├── thehive/                # TheHive + Cassandra + Elasticsearch Docker Compose 배포
│       └── wazuh/                  # Wazuh Manager 설치·MISP 연동
│
└── README.md
```
...
```bash
cd terraform/environments/dev   # 또는 terraform/sec, terraform/ops 등
...
- **PR 생성** → 변경된 `terraform/**` 경로 `fmt check` + `init` + `validate` 자동 실행
...
```bash
cd ansible
# inventory/group_vars/all.yml 에 IP·토큰 값 입력 후
